class ApiService {
  ...
  async login(email, password) {
    // login returns an access token and a refresh token (as a dictionary with keys access and refresh)
  ...
  async refreshAccessToken(refreshToken) {
    // Returns a new access token, if you get a 401 error when making some request, 
    // It might be because access token has expired, you could use an expiring at variable to see when the 
    // access token has expired. The token expires every 60 minutes.
    // if refresh also returns an error, ask the user to login again. Refresh token expires in 365 days.
