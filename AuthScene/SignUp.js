...
export default function SignUp({ navigation }) {
  ...
  let formValidation = () => {
    ...
    if (
      password.match(/^(?=.*[!@#$%^&*])(?=.*[a-z])(?=.*[A-Z]).{8,}$/) == null
    ) {
      error.passwordError =
        "Should atleast contain: 8 characters, 1 Uppercase, 1 Lowercase, and 1 Special character.";
    }
    setError(error);
  ...
  return (
    <SafeAreaView style={signUpStyles.safeContainer}>
      ...
      <Text style={signUpStyles.subHeader}>
        Join us for the best fitness experience.
      </Text>
      <ScrollView keyboardShouldPersistTaps="handled">
        <View style={signUpStyles.container}>
          <Text style={signUpStyles.inputHeader}>Name</Text>
          {errors.userError && (
            <Text style={signUpStyles.error}>{errors.userError}</Text>
          )}
          <TextInput
          ...
          {errors.emailError && (
            <Text style={signUpStyles.error}>{errors.emailError}</Text>
          )}
          ...
          {errors.passwordError && (
            <Text style={signUpStyles.error}>{errors.passwordError}</Text>
          )}
          <View style={signUpStyles.textInputContainer}>
            <TextInput
              style={[signUpStyles.input, { color: "white" }]}
        ...
        <View style={signUpStyles.center}>
          ...
          >
            <Text style={signUpStyles.startedButtonText}>
              Sign Up with Email
            </Text>
          </Pressable>
      ...
      <View style={[signUpStyles.center, { marginBottom: 10 }]}>
        <Pressable
          style={({ pressed }) => [
        ...
        >
          <Text
            style={[signUpStyles.infoText, { textDecorationLine: "underline" }]}
          >
